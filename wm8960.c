...
static int wm8960_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    wm8960 = devm_kzalloc(&client->dev, sizeof(struct wm8960_priv), GFP_KERNEL);
    if (wm8960 == NULL)
        return -ENOMEM;
    ...
    ret = devm_snd_soc_register_component(&client->dev, &soc_component_dev_wm8960, &wm8960_dai, 1);
